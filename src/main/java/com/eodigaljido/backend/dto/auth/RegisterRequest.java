...
@Schema(description = "회원가입 요청 (userId 또는 email 중 하나 이상 필수)")
public record RegisterRequest(
        @Schema(description = "사용자 아이디 (8자 이하, 중복 불가). email 없이 이것만으로 가입 가능.", example = "john123")
        @Size(max = 8, message = "아이디는 8자 이하여야 합니다.") String userId,

        @Schema(description = "이메일 주소 (중복 불가). userId 없이 이것만으로 가입 가능.", example = "user@example.com")
